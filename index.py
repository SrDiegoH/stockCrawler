...
def get_stock_and_reit_from_investidor10(ticker, share_type, info_names):
    try:
        headers = {
...
def convert_investidor10_etf_data(html_page, json_dividends_data, info_names):
    ...
    ALL_INFO = {
        ...
        'market_value': lambda: multiply_by_unit(get_substring(html_page, 'Capitalização</span>', '</span>', patterns_to_remove)),
...
def get_etf_from_investidor10(ticker, share_type, info_names):
    try:
        headers = {
            'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
            'accept-language': 'pt-BR,pt;q=0.9,en-US;q=0.8,en;q=0.7',
...
def get_share_data(ticker, share_type, request_share):
    ...
    data = request_share(ticker, share_type, info_names)
